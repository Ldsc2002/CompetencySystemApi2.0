...
const ItemConsultor = (props) => {
  ...
  const method = async () => {
    if (selectedItem !== "") {
      ...
      const result = state
        ? await props.dispositionsMethod(obj.id)
        : await props.knowledgeElementsMethod(obj.id);
    ...
    <div className={styles.wrapper}>
      ...
      <div>
        <ComboBox
          value={selectedItem}
          options={state
            ? dispositions.map((disposition) => disposition.name)
            : knowledgeElements.map((knowledgeElement) => knowledgeElement.name)
          }
