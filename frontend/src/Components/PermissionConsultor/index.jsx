  ...
  const method = async () => {
    if (transferInputs[0] && transferInputs[1] && selectedCompetency) {
      const response = state
        ? await props.methodCreator(transferInputs[0], transferInputs[1], selectedCompetency.blockId)
        : await props.methodOwner(transferInputs[0], transferInputs[1], selectedCompetency.blockId);
